...
const props = defineProps<DataTableColumnSortProps<Client>>()

const toggleSorting = (columnName: keyof Client) => {
    ...
    <div :class="cn('flex items-center space-x-2', $attrs.class ?? '')">
        <DropdownMenu>
            ...
            <DropdownMenuContent align="start" class="w-56 p-2">
                ...
                <DropdownMenuItem class="flex" v-for="columnName in props.keys" :key="columnName"
                    @click="toggleSorting(columnName)">
                    <span class="flex-grow">
                        {{ columnName }}
                    </span>
                    <Icon icon="ph:arrow-down" v-if="props.table.getColumn(columnName)?.getIsSorted() === 'desc'"
                        class="ml-2 h-4 w-4" />
                    <Icon icon="ph:arrow-up" v-else-if="props.table.getColumn(columnName)?.getIsSorted() === 'asc'"
                        class="ml-2 h-4 w-4" />
