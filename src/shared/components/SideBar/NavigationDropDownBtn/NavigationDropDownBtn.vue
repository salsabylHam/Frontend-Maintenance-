...
<template>
    <Collapsible v-model:open="isOpen">
                ...
                <Tooltip>
                    <TooltipTrigger as-child>
                        <Button variant="ghost" :class="cn(
                            ...
                            <div class="w-6">
                                <span v-if="isRoute()"
                                    class="absolute top-1 left-0 h-8 w-2 rounded-sm bg-black"> </span>
                                <Icon icon="ph:caret-right" v-if="!isOpen"
                                    :class="cn('hidden md:block w-4 h-4 text-accent-foreground ml-1',isRoute()&&'ml-3')">
                                </Icon>
                                <Icon icon="ph:caret-down" v-if="isOpen"
        ...
        <CollapsibleContent class="md:pl-6 space-y-1">
            <NavigationBtn v-for="link in props.links" :icon="link.icon ? link.icon : 'ph:info-fill'" :to="link.to"
                :label="link.label"></NavigationBtn>
        </CollapsibleContent>
    </Collapsible>
</template>
